 ...
 */
function resetTrackingProperties(actionName) {
    if (actionName === void 0) { actionName = 'resetTrackingProperties'; }
    var props = PropertiesService.getScriptProperties();
    // Delete old tracker values (if present)
    var trackerKeys = [SERPAPI_START_INDEX_KEY, STARTUP_ACCEL_CURSOR_KEY];
 ...
 */
function clearSheetDataRows(sheet, actionName) {
 ...
 */
function resetSheets(actionName) {
    if (actionName === void 0) { actionName = 'resetSheets'; }
    var SpreadSheet = SpreadsheetApp.getActiveSpreadsheet();
    var accSheet = SpreadSheet.getSheetByName('accelerators');
    var startupSheet = SpreadSheet.getSheetByName('startups');
    if (accSheet) {
