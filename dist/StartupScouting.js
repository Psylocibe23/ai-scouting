 ...
 */
function inferStartupsFromPortfolioPage(pageUrl, html, accelerator, actionName) {
    if (actionName === void 0) { actionName = 'inferStartupsFromPortfolioPage'; }
    var accelUrlNorm = normalizeUrl(accelerator.website);
    var accelName = accelerator.name || accelUrlNorm || 'Unknown Accelerator';
    var accelCountry = accelerator.country || 'Unknown';
    if (!html) {
        return [];
    }
    // Build structured context for the page + aux pages (about/policy/cookies).
    var pageInfo = buildPageInfo(pageUrl, html, actionName + '.pageInfo');
    // System prompt
    var systemPrompt = [
        // Role
    ...
    userLines.push("- Accelerator website: ".concat(accelUrlNorm || accelerator.website));
    userLines.push("- Accelerator name: ".concat(accelName));
    userLines.push("- Accelerator country: ".concat(accelCountry));
    userLines.push('');
    userLines.push("Page URL being analyzed: ".concat(pageUrl));
    userLines.push('');
    ...
    // Call LLM
    var json = CallLlmJson(systemPrompt, userPrompt, actionName);
    if (!json) {
        AppLogger.warn(actionName, 'LLM returned null/empty JSON, skipping page.', { pageUrl: pageUrl });
        return [];
    }
    var rawList = [];
    ...
    else if (Array.isArray(json.startups)) {
        rawList = json.startups;
    }
    else {
 ...
 *      "Startup X helps Y do W so that Z."
 */
function generateValueProp(startup, actionName) {
    if (actionName === void 0) { actionName = 'generateValueProp'; }
    var rawWebsite = startup.website || '';
    var websiteNorm = normalizeUrl(rawWebsite);
    if (!websiteNorm) {
    ...
    var fetchRes = fetchHtml(websiteNorm, undefined, actionName + '.fetchHtml');
    ...
    var safeName = startup.name && startup.name.trim() !== '' ? startup.name.trim() : websiteNorm;
    ...
    var userPrompt = buildVpPrompt(safeName, websiteNorm, context);
    var json = CallLlmJson(systemPrompt, userPrompt, actionName);
    ...
    // extract fields.
    var startupName = typeof json.startup_name === 'string' ? json.startup_name.trim() : safeName;
    var target = typeof json.target === 'string' ? json.target.trim() : '';
    var what = typeof json.what === 'string' ? json.what.trim() : '';
    var benefit = typeof json.benefit === 'string' ? json.benefit.trim() : '';
    if (!target || !what || !benefit) {
        AppLogger.warn(actionName, 'LLM JSON missing some fields, skipping startup.', { website: websiteNorm });
        return null;
    }
    var sentence = "Startup ".concat(startupName, " helps ").concat(target, " do ").concat(what, " so that ").concat(benefit, ".");
    var update = {
        website: websiteNorm,
...
function generateMissingValueProps(batch_size, actionName) {
    ...
    var candidates = all.filter(function (s) {
        var vp = (s.value_proposition || '').trim();
        return vp === '';
    });
    ...
    var toProcess = candidates.slice(0, batch_size);
    var processed = 0;
    var updated = 0;
    var skippedNoWebsite = 0;
    var skippedError = 0;
    var updates = [];
    toProcess.forEach(function (startup) {
        processed++;
        var rawWebsite = startup.website || '';
        var websiteNorm = normalizeUrl(rawWebsite);
        if (!websiteNorm) {
        ...
        try {
            var upd = generateValueProp(startup, actionName + '.single');
            if (upd) {
                updates.push(upd);
                updated++;
            }
    ...
    var result = {
        processed: processed,
        updated: updated,
        skippedNoWebsite: skippedNoWebsite,
